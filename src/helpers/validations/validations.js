const validateTab1 = (
    ...
    if (name.length < 3) {
      validateCases += "El titulo del problema debe tener al menos 3 caracteres.\n"
    }
    if (difficulty === "") {
      validateCases += "Debes seleccionar una dificultad para el problema.\n";
    }
    if (description === "") {
      validateCases += "Debes agregar una descripción para el problema de al menos 8 caracteres.\n";
    }
    if (solution === "") {
...
const validateTab2 = (
    ...
    templates.forEach(temp => {
      if (temp.code === "") {
        validateCases += "Debes agregar un template al problema en codigo " + temp.language + ".\n";
      }
    });
    if (validateCases !== "") {
      return alert(validateCases);
...
const validateTab3 = (
    ...
    if (problemInfo.testCases.length === 0) {
      validateCases += "Debes agregar al menos un caso de prueba para el problema.\n";
    }
    if (validateCases !== "") {
      return alert(validateCases);
    }
